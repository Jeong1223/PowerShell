...
$Backupdirs #= ("C:\Source1", "C:\Source2") #"C:\Source1", "C:\Source2" #What Folders you want to backup
...
function Write-au2matorLog {
   ...
   # 0 = $directories, 1 = $backupdest, 2 = $backupnumber
...
if ($BackUpCheck) {
    ...
    if ($ZIP) {
        ...
        if ($Use7ZIP) {
            ...
            Compress-Archive -Path $Backupdirs -DestinationPath ($StagingPath + ("\" + $Backupdirs.Replace($Destination, '').Replace('\', '') + ".zip")) -CompressionLevel Optimal -Force 
        }
    }
    else {
