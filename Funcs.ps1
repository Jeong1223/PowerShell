...
#first Fn wait for 100 ...?
Function myPing { ping.exe -w 100 -n 5 10.0.0.13 }
myPing
#ipconfig - find my default gateway ip address
...
Function myNSLookup{ nslook.exe $args}

# myPing (10.0.0.12, 10.0.0.12)  --failed, id address must be specified.n
...
# You can specify the name of the file after notepad, but $$ is shorter and easier. 
# This special variable always contains the last token of
# the last pipeline. In this case, the last token was the name of the file.

notepad $$   # open guidOut.ps1, last element or value
...
Set-Item function:test {
   ...
   Set-Item function:test2 {
    "This function can neither be deleted nor modified if a -force is usedcls
    ."} -option  ReadOnly
   test
   ...
   function Howdy {
    ...
    $args | ForEach-Object { $i++; "$i. Argument: $_" }  # $_: whatever being passed in
    } Else {
...
function Subtract ($Value1, $Value2)
{
 ...
 If ($args.Count -ne 0) {
 Throw "I don't need any more than just two arguments." } #throw produce error message
 $value1 - $value2
}
Subtract 1 2
