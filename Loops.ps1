...
# Attention: danger!
Dir C:\ -recurse | Sort-Object  # It will take a long time
Dir C:\temp -recurse | Sort-Object
...
{
    "A president is {0} {1}" -f $p.FName, $p.LName  #format object
}
...
# Original textbook pg 127 =>foreach
...
$process.GetType().Name # result: Object[]
...
Foreach ($element in Dir C:\) { $element.name }  # the same result with above
