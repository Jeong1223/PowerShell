...
"The size of the file is $($file.Length/1KB) KB bytes."
"The size of the file is $([Math]::Round($file.Length/1KB,3)) KB bytes."

Write-Host($file.length/1KB)
...
# run 
cd C:\temp
.\test1.ps1 # it's empty because of script scope.
#>

<#

ICE 3 - Format-table, creating files, accessing detail of file, creating string variable

dir Variable: | Format-Table Name, Value, Description, options >> C:\temp\vars.txt
...
$list.servers.server | ? { $_.user -eq "Bob" } | Select-Object ip
